...
- **Finish a bounded stack together.** Review each layer independently, then merge the planned set once every layer and the combined result are ready. Keep Codex review, CI, and human approval distinct. New work must not keep extending a stack waiting to land.
