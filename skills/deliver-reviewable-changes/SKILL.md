...
Use sequential PRs or a shallow stack for dependent changes, following the task's authorization and repository workflow. Keep the active chain short, base each layer on its actual prerequisite, and put review fixes in the layer that owns the concern. For a stack, define the planned top and aim to merge the complete finished stack together after every layer is ready. A lower layer that is ready waits for that bounded set; unrelated future work belongs in a separate delivery. Each layer must still be independently safe after its prerequisites. Do not rewrite shared history or start additional workers merely to apply this skill.
...
For a stack, confirm that every PR in the planned set has finished implementation, addressed blocking findings, obtained current reviewer approval, and passed all applicable CI checks before starting the coordinated merge. When Codex review is the designated automated reviewer, obtain its approval for each candidate and track it separately from human approval. Also validate the combined result at the top of the stack. Use the repository's supported whole-stack merge or queue workflow; follow its dependency order and actual atomicity guarantees. A new commit or invalidated gate requires refreshed readiness, even after some layers have landed. Read [delivery slices](references/delivery-slices.md) for the review and merge handoff.
