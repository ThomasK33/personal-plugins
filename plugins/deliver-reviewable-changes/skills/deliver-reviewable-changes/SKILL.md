...
description: Read before planning or executing substantial features, refactors, migrations, or other code changes spanning multiple components or behaviors, even when no PR is mentioned. Helps prevent oversized PRs and repeated review rework by defining independently shippable increments, validation, and human review decisions early. Also use when scope grows during implementation or when preparing an existing PR.
---
...
Use sequential PRs or a shallow stack for dependent changes, following the task's authorization and repository workflow. Keep the active chain short, base each layer on its actual prerequisite, and put review fixes in the layer that owns the concern. A ready lower layer should proceed through its required approval and release gates without waiting for the milestone. A stack must not become one large, inseparable release. Do not rewrite shared history or start additional workers merely to apply this skill.
